<script>
    ...
    const schema = yup.object().shape({
        providergame_id_field: yup
            ...
            .max(10, "ID must be at most 10 Character"),
        providergame_name_field: yup
            .string()
        ...
        providergame_phone_field: yup
            .string()
            .required("Phone is Required")
            .matches(
                /^[0-9\-\+ ]+$/,
                "Phone must Character  1-9"
            )
            .min(5, "Phone must be at least 4 Character")
            .max(30, "Phone must be at most 30 Character"),
        providergame_email_field: yup
            .string()
            .required("Email is Required")
            .min(4, "Email must be at least 4 Character")
            .max(350, "Email must be at most 350 Character"),
        providergame_note_field: yup.string(),
        providergame_status_field: yup.string().required("Status is Required"),
    });
    const { form, errors, handleChange, handleSubmit } = createForm({
        initialValues: {
            providergame_id_field: "",
            providergame_name_field: "",
            providergame_phone_field: "",
            providergame_email_field: "",
            providergame_note_field: "",
            providergame_status_field: "",
        },
        validationSchema: schema,
        onSubmit: (values) => {
            SaveTransaksi(
                values.providergame_id_field,
                values.providergame_name_field,
                values.providergame_phone_field,
                values.providergame_email_field,
                values.providergame_note_field,
                values.providergame_status_field
            );
        },
    });
    async function SaveTransaksi(idcate, name,phone,email,note, status) {
        let flag = true;
        msg_error = "";
        if(status == ""){
            flag = false;
        ...
        if (flag) {
            ...
            loader_msg = "Sending..."
            const res = await fetch(path_api+"api/saveprovidergame", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    sdata: sData,
                    page: "PROVIDERGAME-SAVE",
                    idprovidergame: idcate.toUpperCase(),
                    name: name,
                    phone: phone,
                    email: email,
                    note: note,
            ...
            if(!res.ok){
                ...
                if (json.status == 200) {
                    ...
                    if(sData == "New"){
                        $form.providergame_id_field = "";
                        $form.providergame_name_field = "";
                        $form.providergame_phone_field = "";
                        $form.providergame_email_field = "";
                        $form.providergame_note_field = "";
                        $form.providergame_status_field = "";
                        $errors.providergame_id_field = "";
                        $errors.providergame_name_field = "";
                        $errors.providergame_phone_field = "";
                        $errors.providergame_email_field = "";
                        $errors.providergame_note_field = "";
                        $errors.providergame_status_field = "";
                    }
...
<Modal_popup
    ...
    modal_popup_class="select-none max-w-full max-w-2xl overflow-hidden">
        ...
        <div class="grid grid-cols-2 gap-3 mt-2 ">
            <div>
                <Input_custom
                    ...
                    input_invalid={$errors.providergame_id_field.length > 0}
                    bind:value={$form.providergame_id_field}
                    input_id="providergame_id_field"
                    input_enabled={providergame_id_enabled}
                    input_text_class="uppercase"
                    input_placeholder="ID"/>
                {#if $errors.providergame_id_field}
                    <small class="text-pink-600 text-[11px]">{$errors.providergame_id_field}</small>
                {/if}
            </div>
            <div>
                <Input_custom
                    ...
                    input_invalid={$errors.providergame_name_field.length > 0}
                    bind:value={$form.providergame_name_field}
                    input_id="providergame_name_field"
                    input_placeholder="Nama"/>
                {#if $errors.providergame_name_field}
                    <small class="text-pink-600 text-[11px]">{$errors.providergame_name_field}</small>
                {/if}
            </div>
            <div>
                <Input_custom
                    ...
                    input_invalid={$errors.providergame_phone_field.length > 0}
                    bind:value={$form.providergame_phone_field}
                    input_id="providergame_phone_field"
                    input_placeholder="Phone"/>
                {#if $errors.providergame_phone_field}
                    <small class="text-pink-600 text-[11px]">{$errors.providergame_phone_field}</small>
                {/if}
            </div>
            <div>
                <Input_custom
                    ...
                    input_invalid={$errors.providergame_email_field.length > 0}
                    bind:value={$form.providergame_email_field}
                    input_id="providergame_email_field"
                    input_placeholder="Email"/>
                {#if $errors.providergame_email_field}
                    <small class="text-pink-600 text-[11px]">{$errors.providergame_email_field}</small>
                {/if}
            </div>
            <div>
                <textarea 
                    on:change={handleChange}
                    invalid={$errors.providergame_note_field.length > 0}
                    bind:value={$form.providergame_note_field}
                    class="textarea textarea-bordered w-full" placeholder="Note"></textarea>
                {#if $errors.providergame_note_field}
                    <small class="text-pink-600 text-[11px]">{$errors.providergame_note_field}</small>
                {/if}
            </div>
            <div class="">
                <select
                    on:change="{handleChange}"
                    bind:value={$form.providergame_status_field}
                    invalid={$errors.providergame_status_field.length > 0} 
                ...
                {#if $errors.providergame_status_field}
                    <small class="text-pink-600 text-[11px]">{$errors.providergame_status_field}</small>
                {/if}
            </div>
            {#if sData == "Edit"}
                <Panel_info panel_body_class="col-span-2 mb-2">
                    <slot:template slot="panel_body">
                        <table>
                            <tr>
                                <td>Create</td>
