<script>
    import Home from "../banktype/Home.svelte";
    ...
    async function initHome() {
        listHome = []
        home_listcatebank = []
        const res = await fetch(path_api+"api/banktype", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: "Bearer " + token,
        ...
        if (!res.ok) {
            ...
            if (json.status == 200) {
                record = json.record;
                let recordlistcatebank = json.listcatebank;
                let status_class = "";
                let status_text = "";
                let no = 0;
                if (record != null) {
                    ...
                    for (var i = 0; i < record.length; i++) {
                        ...
                        listHome = [
                            ...
                            {
                                home_no: no,
                                home_id: record[i]["banktype_id"],
                                home_idcatebank: record[i]["banktype_idcatebank"],
                                home_nmcatebank: record[i]["banktype_nmcatebank"],
                                home_nama: record[i]["banktype_name"],
                                home_img: record[i]["banktype_img"],
                                home_status: status_text,
                                home_create: record[i]["banktype_create"],
                                home_update: record[i]["banktype_update"],
                                home_statusclass: status_class,
                            },
                        ];
                    }
                }
                if (recordlistcatebank != null) {
                    for (let i = 0; i < recordlistcatebank.length; i++) {
                        home_listcatebank = [
                            ...home_listcatebank,
                            {
                                catebank_id:recordlistcatebank[i]["catebank_id"],
                                catebank_name:recordlistcatebank[i]["catebank_name"]
                            },
                        ];
                    }
                }
